...
import { FiUploadCloud, FiCheck, FiAlertCircle, FiCode, FiX } from 'react-icons/fi';
...
export const EnvImportModal: React.FC = () => {
  ...
  const resetState = () => { setStep('input'); setIsDragging(false); setPasteContent(''); setParsedSecrets([]); setTargetEnv('Development'); setTargetProject(''); };
  ...
  const parseContent = (content: string) => {
    ...
    for (const line of lines) { const t = line.trim(); if (!t || t.startsWith('#')) continue; const idx = t.indexOf('='); if (idx > 0) { const k = t.substring(0, idx).trim(); let v = t.substring(idx + 1).trim(); if ((v.startsWith('"') && v.endsWith('"')) || (v.startsWith("'") && v.endsWith("'"))) v = v.slice(1, -1); secrets.push({ key: k, value: v }); } }
    if (secrets.length > 0) { setParsedSecrets(secrets); setStep('review'); }
  };
  const handleFile = (file: File) => { const r = new FileReader(); r.onload = e => { const t = e.target?.result as string; if (t) parseContent(t); }; r.readAsText(file); };
  const handleImport = async () => {
    if (!user || parsedSecrets.length === 0) return; setStep('importing');
    for (const s of parsedSecrets) { await createSecret(user.uid, { name: s.key, value: s.value, service: 'Other', type: 'Other', environment: targetEnv, status: 'Active', notes: 'Imported from .env', lastRotated: null, expiresOn: null, projectId: targetProject || null }); }
    setStep('success'); setTimeout(handleClose, 2000);
  };
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 8 }} transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1] }} style={{ position: 'relative', width: '100%', maxWidth: 440, background: 'var(--color-surface)', border: '1px solid var(--color-border-2)', boxShadow: '0 24px 64px rgba(0,0,0,.7)', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '85vh' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: '1px solid var(--color-border)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 30, height: 30, background: 'var(--color-surface-2)', border: '1px solid var(--color-border)', display: 'grid', placeItems: 'center' }}><FiUploadCloud size={14} color="var(--color-green)" /></div>
                <div>
                  <h2 style={{ fontSize: 14, fontWeight: 700, color: 'var(--color-text)', margin: 0 }}>{step === 'review' ? 'Review Secrets' : step === 'paste' ? 'Paste .env Content' : 'Import .env File'}</h2>
                  <p style={{ fontSize: 11, color: 'var(--color-text-2)', margin: '2px 0 0 0' }}>{step === 'review' ? `${parsedSecrets.length} keys ready` : 'Sync your config securely'}</p>
                </div>
              </div>
              {step !== 'importing' && step !== 'success' && <button onClick={handleClose} style={{ width: 28, height: 28, display: 'grid', placeItems: 'center', border: '1px solid var(--color-border)', background: 'none', color: 'var(--color-text-2)', cursor: 'pointer' }}><FiX size={14} /></button>}
            </div>

            <div style={{ padding: 18, overflowY: 'auto', flex: 1 }} className="hide-scrollbar" data-lenis-prevent="true">
              {step === 'input' && (
                <div>
                  <div onDragOver={e => { e.preventDefault(); setIsDragging(true); }} onDragLeave={e => { e.preventDefault(); setIsDragging(false); }} onDrop={e => { e.preventDefault(); setIsDragging(false); const f = e.dataTransfer.files?.[0]; if (f) handleFile(f); }} onClick={() => fileInputRef.current?.click()}
                  ...
                  <button onClick={() => setStep('paste')} style={{ width: '100%', padding: 12, border: '1px solid var(--color-border)', background: 'none', color: 'var(--color-text-2)', fontSize: 13, fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, cursor: 'pointer', fontFamily: 'var(--font-sans)' }}><FiCode size={14} /> Paste raw config text</button>
                </div>
              )}

              {step === 'paste' && (
                <div>
                  <textarea value={pasteContent} onChange={e => setPasteContent(e.target.value)} placeholder={'# Paste contents here...\nAPI_KEY=sk-proj-abc123'} autoFocus style={{ width: '100%', height: 200, padding: 12, border: '1px solid var(--color-border)', background: 'var(--color-bg)', color: 'var(--color-text)', fontFamily: 'var(--font-mono)', fontSize: 12, outline: 'none', resize: 'none', marginBottom: 16, boxSizing: 'border-box' }} />
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={() => parseContent(pasteContent)} disabled={!pasteContent.trim()} style={{ padding: '7px 18px', background: 'white', color: '#080808', border: 'none', fontSize: 12, fontWeight: 700, cursor: !pasteContent.trim() ? 'not-allowed' : 'pointer', opacity: !pasteContent.trim() ? 0.5 : 1, fontFamily: 'var(--font-sans)' }}>Parse Secrets</button>
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', maxHeight: 180, overflowY: 'auto' }} className="hide-scrollbar">
                    {parsedSecrets.map((s, i) => (
                      <div key={i} style={{ padding: '8px 10px', borderBottom: '1px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--color-green)', background: 'var(--color-green-bg)', padding: '1px 5px' }}>{s.key}</span>
                        <span style={{ fontSize: 11, color: 'var(--color-text-3)' }}>{s.value.replace(/./g, '•')}</span>
                      </div>
                    ))}
                  </div>
                  <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: 'rgba(59,130,246,0.03)', border: '1px solid rgba(59,130,246,0.1)' }}>
                  ...
                  <div style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <button onClick={() => { setParsedSecrets([]); setStep('input'); }} style={{ background: 'none', border: 'none', color: 'var(--color-red)', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'var(--font-sans)' }}><FiX size={12} /> Discard</button>
                    <button onClick={handleImport} style={{ padding: '8px 18px', background: 'white', color: '#080808', border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>Import {parsedSecrets.length} Secrets</button>
                  </div>
                </div>
              )}
                ...
                <div style={{ padding: '32px 0', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  {step === 'importing' ? (
                    <>
                      ...
                      <p style={{ fontSize: 12, color: 'var(--color-text-2)', marginTop: 6, textAlign: 'center' }}>Creating {parsedSecrets.length} entries...</p>
                    </>
                  ) : (
                    <>
                      <div style={{ width: 48, height: 48, background: 'var(--color-green-bg)', border: '1px solid var(--color-green-border)', display: 'grid', placeItems: 'center', marginBottom: 16 }}><FiCheck size={24} color="var(--color-green)" /></div>
                      <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--color-text)', margin: 0 }}>Import Complete</h3>
                      <p style={{ fontSize: 12, color: 'var(--color-text-2)', marginTop: 6 }}>Secrets are safely in the vault.</p>
                    </>
                  )}
                </div>
              )}
