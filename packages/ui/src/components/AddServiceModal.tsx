...
import { CustomServiceIcon, CUSTOM_SERVICE_ICON_KEYS, CUSTOM_SERVICE_ICON_MAP } from './CustomServiceIcons';
...
export const AddServiceModal: React.FC = () => {
  ...
  const [icon, setIcon] = useState('FaAmazon');
  const [color, setColor] = useState<ProjectColor>('green');
  const [isSubmitting, setIsSubmitting] = useState(false);

  const resetForm = () => { setName(''); setIcon('FaAmazon'); setColor('green'); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createService(user.uid, { name: name.trim(), icon: icon || 'FaAmazon', color });
      handleClose();
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: '1px solid var(--color-border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--color-text-2)', margin: '2px 0 0 0', fontFamily: 'var(--font-sans)' }}>For providers not listed</p>
                </div>
              </div>
            ...
            <div style={{ padding: 18, maxHeight: '80vh', overflowY: 'auto' }}>
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  ...
                  <input required maxLength={24} value={name} onChange={e => setName(e.target.value)} style={inputStyle} placeholder="e.g. MyLocalDB" autoFocus onFocus={e => e.currentTarget.style.borderColor = 'var(--color-border-focus)'} onBlur={e => e.currentTarget.style.borderColor = 'var(--color-border)'} />
                </div>
                ...
                <div>
                  ...
                  <div data-lenis-prevent="true" style={{
                    ...
                    {CUSTOM_SERVICE_ICON_KEYS.map(key => {
                      const isSelected = icon === key;
                      const Icon = CUSTOM_SERVICE_ICON_MAP[key];
                      return (
                        <button
                          key={key}
                          type="button"
                          title={key.replace('Si', '').replace('Fa', '')}
                          onClick={() => setIcon(key)}
                          style={{
                            width: 34, height: 34,
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, color: 'var(--color-text-3)', fontSize: 11 }}>
                    <div style={{ width: 24, height: 24, display: 'grid', placeItems: 'center', background: `${accentColor}18`, border: `1px solid ${accentColor}40` }}>
                      <CustomServiceIcon iconKey={icon} size={13} color={accentColor} />
                    </div>
                    <span style={{ fontFamily: 'var(--font-mono)' }}>{icon.replace('Si', '').replace('Fa', '')} selected</span>
                  </div>
                </div>
